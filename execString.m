function execString(handles)
    ...
    try
        ...
        if (islogical(size(a) ~= 0)) && (islogical(size(b) ~= 0)) && (islogical(size(tol) ~= 0)) && (islogical(size(mxCount) ~=0)) && (islogical(emptyness ~= 1))
            [solution, counts] = stringMethod(hndlAxes, hndlTable, fnct, a, b, tol, mxCount);
        else
        ...
        % End
        updateInformationBox(hndlInformationBox, 'End string.');
        % When the excecution ends, we keep a record to our log file
        trcLogger('The string method ended.');
        if webTraceFlag == 1
            webLog('The string method ended.', 'event');
        end
    catch exc
